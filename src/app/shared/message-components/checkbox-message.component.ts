...
export class CheckBoxMessageComponent implements OnInit {
    ...
    @Output() public NewEntryAdded = new EventEmitter();
    header = '';
    options: string[];
    ...
    onSelectionChange(event: any, option: any) {
        if (event.target.checked) {
            ...
            const updateItem = this.selectedOption.find(this.findIndexToUpdate, option);
            const index = this.selectedOption.indexOf(updateItem);
