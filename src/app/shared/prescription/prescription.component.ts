...
export class PrescriptionComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    medicineNumberInc() {
      ...
      const items = this.prescriptionForm.get('medication') as FormArray;
    ...
    numberDec(parameter: String) {
        ...
        if (parameter === 'medicine')  {
            ...
            const items = this.prescriptionForm.get('tests') as FormArray;
            items.removeAt(items.length - 1);
        }
    }
    testsNumberInc() {
    ...
    createMedicalItem(): FormGroup {
      ...
      generatePres(prescriptionData: any) {
        ...
        let i: number;
        const prescription: any = prescriptionData.value;
        const issue: string = prescription.issue || 'NA';
        const symptoms: string = prescription.symptoms || 'NA';
        let medication: Array<any> = [];
        let tests: Array<any> = [];
        const specialInstructions: string = prescription.specialInstructions || 'NA';
    try {
    if (prescription.medication[0].name) {
        for (i = 0; i < prescription.medication.length; i++) {
            ...
            prescription.medication[i].time3 === true ? prescription.medication[i].time3 = 'After Food'
            : prescription.medication[i].time3 = null;
            prescription.medication[i].time4 === true ? prescription.medication[i].time4 = 'Before Food'
            : prescription.medication[i].time4 = null;
    const array = {
    'id': i + 1,
   ...
   generatePdf(issuePassed: any, symptomsPassed: any, medication: any, tests: any, specialInstructions: any,
     doctorName: any, doctorDetailsPassed: any) {
    const medicineList: string[] = [];
    const diagnosticList: string[] = [];
    ...
    const prescriptionInfo = {
      ...
      const table3PosY = table1Pos.y > doc.autoTable.previous.finalY ? table1Pos.y : doc.autoTable.previous.finalY;
      ...
      this.drawCanvas(this.digitalSignature, 185, 80).then((res) => {
        ...
        this.drawCanvas(this.rxLogoImage, 28, 28).then((res2) => {
            doc.addImage(res2, 'JPG', 2, 100);
            doc.setFillColor(255, 140, 0);
            doc.rect(0, 790, 595, 841, 'F');
            this.drawCanvas(this.mesomedslogoImage, 90, 90).then((res1) => {
            doc.addImage(res1, 'JPEG', 10, 10);
